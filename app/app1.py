    ...
    if tab == 'tab-1':
                        ...
                        html.Div([html.H2('Proyecciones Rankings Generales',style={'justify-content':'center',
                                  ...
                                  html.Div([
                                      html.Div([dcc.Checklist(id='rkweapon-picker',
                                                              options=[{'label':i,'value':i}for i in rank['weapon'].unique()],
                                                              value=['epee','sabre','foil'],
                                                             labelStyle = {'margin-left':'25px'})
                                      ],style={'justify-content':'center','display':'flex', 'vertical-align': 'middle',
                                          'width':'50%'}),
                                      html.Div([dcc.Checklist(id='rkgender-picker',
                                                              options=[{'label':i,'value':i}for i in rank['gender'].unique()],
                                                              value=['fem','masc'],
                                                             labelStyle = {'margin-left':'25px'})
                                      ],style={'justify-content':'center','display':'flex', 'vertical-align': 'middle',
                                          'width':'50%'})
                            ...
                            html.Div([html.H2(id='rank-range-select',style={'justify-content':'center',
                                      ...
                                      dcc.RangeSlider(id='rkrank-picker',
                                                      marks={1:'#1',10:'Top 10',32:'Top 32',100:'Top 100',156:'+++'},
                                                      min=1,
                                                      max=156,
                                                      value=[1,100])
        ...
        return html.Div([
            html.Div([dash_table.DataTable(columns=[{"name": i, "id": i} for i in df.groupby(['name','country','weapon','gender'],as_index=False).agg({'age':'max',
                                                            ...
                                                            'PM1V%':'mean',
                                                            'PM1&2V%':'mean'}).columns],
                                   data=df.groupby(['name','country','weapon','gender'],as_index=False).agg({'age':'max',
                                                            'comp':'count',
                                                            'POS':'mean',
                                                            'T96+':'sum',
                                                            'T64+':'sum',
...
def update_graph(esg,weapons,genero,value_list):
    ...
    return dict(data=traces,layout=go.Layout(title='Proyecciones del Ranking',xaxis=dict(title='Edad'),yaxis=dict(title='Ranking')))
