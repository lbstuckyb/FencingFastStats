...
- [ ] M2 — One competition end-to-end (partial — resume in a fresh session)
  ...
  - **Not yet done (next session must still do these before the M2 ⛔ STOP):** wire `ffs scrape --comp 2025/242 [--dump]` into `cli.py`; write to `data/canonical/*.parquet`; freeze 2–3 real payloads as `tests/fixtures/*.json.gz` (suggest: 2025/242 already explored above, plus one older/smaller comp, plus one with a real forfeit — comp 2025/242 itself has one DE forfeit, pool GIANNOTTE/JORGENSEN `B64`, so it alone may suffice for a forfeit fixture); write `pytest` suite (`test_devalue.py`, `test_parse.py`) including the golden check already manually confirmed above (entry count 215, champion KANO Koki 34385, one full poule, the B2 final). Nothing has been committed yet — all four new files under `src/ffs/` are untracked.
