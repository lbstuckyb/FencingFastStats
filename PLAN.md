...
- [ ] M5 — Site data + core pages (IN PROGRESS, partial commit)
  ...
  - **Next session should:** read this note + `build_site.py`'s module docstring first (don't re-derive the JSON shapes), run `ffs build-site` if `site/data/` isn't present locally (gitignored, regenerate on demand, ~6.5 min), then build the site shell + 3 pages per the plan, verify via `python -m http.server -d site`, then the real M5 commit (`feat: static site core (home, search, fencer profiles)`).
