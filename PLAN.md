...
- [ ] M3b — Bulk historical scrape (partial — driver built + verified, full scrape NOT yet run)
  - **Built and working, not yet executed at scale.** Added to `cli.py`: `_scrape_one()` (factored out of `cmd_scrape` — fetch+decode+parse+`validate_tables()`, raises `ValueError` on any issue), `_existing_competition_ids()` (reads `competition_id` col from `data/canonical/competitions.parquet` if it exists), `cmd_scrape_all()` wired to a new `ffs scrape-all --from YYYY --to YYYY [--weapon] [--gender] [--force] [--max-consecutive-fails N]` subcommand.
  - **Design:** iterates seasons in range (checked against `fetch_seasons()`), for each season calls `list_competitions()` (Senior/Individual, all weapons/genders by default — confirmed **3092 total competitions across 2002–2026** via a dry-run count using `discover.list_competitions`, no scraping). Per competition: skip if `competition_id` already in `data/canonical/competitions.parquet` (resumable — re-running the same command after a crash just skips already-flushed comps; `FieClient`'s gz cache also means re-fetching already-cached HTTP responses is instant, no network/throttle cost). Tables are accumulated **in memory per season** and flushed to parquet once per season (not per competition) to bound read/concat/write overhead across ~3000 comps — a mid-season crash loses that season's unflushed progress but it replays fast from the HTTP cache on retry. Per-competition failures are caught, logged to `data/raw_cache/scrape_failures.json`, and the run continues; it aborts early (exit 1) after `--max-consecutive-fails` (default 15) consecutive failures, to fail fast on a real outage rather than grinding through thousands of doomed attempts.
  - **Decided the M3a open question:** zero-result competitions (like `2004/433`) are **kept as rows** (empty athletes/results/bouts, populated `competitions` row) rather than skipped, for browsing completeness — `_scrape_one` already handles this cleanly per M3a's shape survey.
  - Wrote `validate_legacy.py`: `load_legacy_competitions()` (groups the legacy CSV by `(comp, place, date, weapon, gender)` — **286 distinct competition-events**, not the plan's earlier "204" estimate, which was likely counting by `(comp, place, date)` only = 229; 286 is the correct granularity since canonical `competitions` rows are already split by weapon/gender), `match_competitions()` (weapon+gender exact, `start_date` within ±1 day), `normalize_name()` (accent/case/punctuation-stripping helper, unused yet — reserved for M4's fencer-level parity matching, since legacy ids are lossy `name[:11].lower()` slugs). Wired to `ffs validate` in `cli.py` (`cmd_validate`), prints match count/rate and lists unmatched legacy events; exits 1 if match rate < 95%.
  - **Verified working, not yet meaningfully run:** `ffs validate` executes cleanly against the 14 competitions already in `data/canonical/` from prior-session testing (1/286 matched, 0.3% — expected and correct, since those 14 barely overlap the legacy CSV's 2015–2022 window). `pytest` still 34/34 green (no new tests added yet for `validate_legacy.py` — TODO next session, plus a small `scrape-all` resume/skip unit test if feasible without hitting network).
  - **Session 3 (2026-07-14, later same day) — closed out steps 1–3, full scrape IN PROGRESS when session ended:**
    - Added `tests/test_validate_legacy.py` (normalize_name, `load_legacy_competitions` row count against the real committed CSV — 286, `match_competitions` exact/tolerance/no-match cases) and `tests/test_cli_scrape_all.py` (`_existing_competition_ids` missing-file/reads-column). **41/41 tests pass.**
    - Smoke-tested `ffs scrape-all --from 2002 --to 2002`: 6/6 comps ok, 0 failures, ~26s. Confirmed season-flush and re-run-skips-all-6 (resumability) both work against live fie.org. Confirmed pre-2016 comps parse as results-only (0 bouts, populated results) as expected from M3a.
    - **Launched the full run**: `nohup ffs scrape-all --from 2002 --to 2026 > data/raw_cache/scrape_all.log 2>&1 &` + `disown`, started ~21:23 on 2026-07-14, PID **18988** (python; parent shell PID 18985). This process is fully detached (`nohup` + `disown`) — it survives this session ending and does NOT need to be restarted or resumed; it just needs to be checked on.
    - Progress at session-end (~1h45m in): season 2012, **1382 ok, 0 failures** so far (out of 3092 total across 2002–2026, minus 6 already done for 2002 in the smoke test). Rate held steady around 13–14 comps/min with zero failures throughout — no signs of throttling/blocking issues.
    - **Explicit next steps for the resuming session (in order):**
      1. First check whether PID 18988 is still running (`ps -p 18988`) and tail `data/raw_cache/scrape_all.log`. If still running, either wait for it (background-watch it, e.g. `while kill -0 18988 2>/dev/null; do sleep 60; done`) or just come back later — do not start a second `scrape-all` run concurrently (would double-hit fie.org and race on the same parquet files). If it already finished (or died), running `ffs scrape-all --from 2002 --to 2026` again is safe and resumes cleanly (skips everything already in `data/canonical/competitions.parquet`).
      2. Check `data/raw_cache/scrape_failures.json` — if any failures accumulated, retry those specifically (`ffs scrape --comp SEASON/ID` per straggler, or just re-run `scrape-all` which will retry anything not yet in canonical parquet).
      3. Run `ffs validate` for the real coverage report; target ≥95% legacy-competition match rate per the plan's Verify line. Investigate systematic misses (e.g. a weapon/gender code mismatch, or events legacy has that predate/postdate fie.org's digitized archive) rather than just accepting a low number.
      4. Decide whether `data/canonical/*.parquet` is small enough to commit (~<50MB per the plan) or stays gitignored with a size note in the Progress Log — check with `du -sh data/canonical/`.
      5. THEN commit (including the two new test files above, still untracked) as `data: full historical scrape + canonical tables` and mark M3b's checkbox `[x]` (replace this partial entry's heading), per the plan's normal one-milestone-per-session stop protocol.
- [ ] M4 — Stats + ELO + H2H
