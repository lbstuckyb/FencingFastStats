...
- [ ] M3b — Bulk historical scrape (partial — driver built + verified, full scrape NOT yet run)
  ...
  - **Explicit next steps for the resuming session (in order):**
    1. Optionally add unit tests for `validate_legacy.load_legacy_competitions`/`match_competitions` (pure logic, no network — use the real committed legacy CSV) and skip-logic tests for `cmd_scrape_all` helpers.
    2. Smoke-test `ffs scrape-all --from 2002 --to 2002` (only 6 comps) end-to-end to confirm the season-flush + skip + failure paths all work against live fie.org before committing to the full range.
    3. Run `ffs scrape-all --from 2002 --to 2026` for real — **expect hours of wall-clock** (3092 comps × ~2 requests × 1.5s throttle ≈ 2.5h floor, plus parse/write overhead). Run via a backgrounded shell (`nohup`/`&` or the harness's background-bash equivalent), redirecting stderr progress to a log file (e.g. `data/raw_cache/scrape_all.log`) so it survives a session boundary; if it dies partway, just re-run the identical command — it resumes from `data/canonical/competitions.parquet`'s existing rows, no flags needed. Check `data/raw_cache/scrape_failures.json` afterward and retry those specifically if any (rerun `ffs scrape-all` again, or `ffs scrape --comp` for individual stragglers) before considering the run done. **Do not run this synchronously in an interactive turn — it will not finish inside a normal tool timeout.**
    4. Once the scrape is complete (or "done enough" — no more progress after retries), run `ffs validate` for the real coverage report; target ≥95% legacy-competition match rate per the plan's Verify line. Investigate systematic misses (e.g. a weapon/gender code mismatch, or events legacy has that predate/postdate fie.org's digitized archive) rather than just accepting a low number.
    5. Decide whether `data/canonical/*.parquet` is small enough to commit (~<50MB per the plan) or stays gitignored with a size note in the Progress Log — check with `du -sh data/canonical/`.
    6. THEN commit as `data: full historical scrape + canonical tables` and mark M3b's checkbox `[x]` (replace this partial entry's heading), per the plan's normal one-milestone-per-session stop protocol.
- [ ] M4 — Stats + ELO + H2H
